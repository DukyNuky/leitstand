...
| **TeamSpeak über UDP** | der Handschlag, mit dem jeder Client anfängt (`Init1`), statt eines Pakets ins Dunkle: bei UDP beweist ein ausbleibendes Paket nichts — kein Server, ein schweigender Server und eine Firewall sehen gleich aus. Geantwortet hat er oder nicht, und hört auf dem Port gar nichts zu, sagt die Meldung auch das (ICMP „Port unreachable“). Ab Werk 9987, jeder andere Port per `ts3:19987` |
...
| **Sicherungsaufträge** | die in Proxmox VE eingerichteten `vzdump`-Aufträge je Knoten: Zeitplan, Ziel, Umfang, nächster Lauf — und **ob und wann sie gelaufen sind**: letzter Lauf mit Ausgang, letzter erfolgreicher, letzter fehlgeschlagener, jeder in eigener Spalte. Bewertet wird der *letzte* Lauf; ein überstandener Fehlschlag bleibt sichtbar, ohne zu alarmieren. Der **Umfang nennt die Gäste beim Namen** — der Auftrag kennt nur Nummern, die Namen stehen in der Bestandsliste des Knotens. Hat ein Auftrag in Proxmox keinen Kommentar, hat er hier keinen Namen: die Kennung `backup-0011aabb-ccdd` ist keine Bezeichnung, und sie als eine auszugeben hilft niemandem |
...
| **OPNsense** | Fassung und offene Aktualisierungen, Laufzeit und Last, Arbeitsspeicher, Platte, WireGuard-Peers mit Handshake-Alter · **die Adressen nach außen**: je Anschluss die eigene und jeder **IP-Alias** auf derselben Schnittstelle, CARP-Adressen als solche gekennzeichnet — als Anschluss nach außen gilt, woran ein Gateway hängt, nicht was „WAN" heißt · **jede Schnittstelle einzeln**: Durchsatz ↓/↑, Pakete/s, übertragene Menge, Fehler und Verwürfe (Stand und Zuwachs), Verbindungszustand und MTU, mit Verlauf über Tage je Leitung · **Gateways** mit Zustand, Latenz und Verlust — die Meldung, die von außen niemand sieht, weil die Firewall dabei tadellos antwortet · **Zustandstabelle** und **CARP**-Rolle |
...
| **WAN-Adresse gelesen statt getippt** | Am Standort stand sie bisher so, wie sie jemand einmal eingetragen hat — bei einem Anschluss, der sich täglich eine neue holt, nach 24 Stunden eine Zahl ohne Bedeutung. Jetzt liest die Firewall sie selbst, und **beides steht nebeneinander**: läuft das Eingetragene vom Gemessenen weg, sagt die Oberfläche es. Eine private Adresse am WAN wird als solche gekennzeichnet — dahinter hängt ein Modem-Router, der die eigentliche trägt
| **Prüfungen zum Anhaken** | Was an einem System geprüft wird, steht sonst aus IP und Adresse geraten fest — ICMP, der Port der Oberfläche, bei https das Zertifikat. Für Proxmox stimmt das, für ein **sonstiges** Gerät selten: ein Switch, der nur SSH spricht, leuchtet für immer gelb wegen eines nie vorhandenen Ports. Unter *Verwaltung → System bearbeiten* lässt sich die Liste deshalb selbst zusammenklicken — ICMP, HTTP, HTTPS, Zertifikat, SSH, DNS, TeamSpeak, SMTP, IMAP, RDP, SMB, Drucker, Datenbanken, dazu ein Feld für alles Übrige: eine nackte Zahl ist ein TCP-Port, mit einem Verfahren davor geht auch `ts3:19987` für einen Dienst auf einem eigenen Port. **Eine Zeile ist genau eine Prüfung**, auch das Zertifikat: eine Liste, die beim Anzeigen etwas hinzuerfindet, schreibt es beim nächsten Speichern in die Datei. Was mehr trägt als Art und Port (ein eigener `servername`, eine eigene DNS-Abfrage) bleibt unangetastet stehen |
...
**Proxmox Mail Gateway kennt keine API-Token.** Das ist der Fallstrick dieser
Anbindung: seine API-Dokumentation weist Token an jedem Endpunkt als erlaubt aus
— die Beschreibung wird aus derselben Vorlage erzeugt wie die von Proxmox VE —,
sein HTTP-Dienst weist sie aber vor jeder Rechteprüfung ab
(`die "API tokens not implemented"`, `src/PMG/HTTPServer.pm`). Wer eine
Token-Kopfzeile schickt, bekommt eine wortlose 401 und sucht danach am falschen
Ende. Angemeldet wird deshalb wie an der Oberfläche: Benutzer und Passwort gegen
`/access/ticket`, danach das Ticket als Cookie `PMGAuthCookie`. Es gilt zwei
...
7. OPNsense: die HAProxy-Backends — Schnittstellen, Gateways,
   ...
   Benutzer auf
8. Alarm-Postfach anbinden (IMAP IDLE + Regelwerk)
9. **Push-Kanäle und Totmannschalter** — solange die fehlen, muss jemand hinsehen
