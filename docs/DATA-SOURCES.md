...
| Endpunkte | `/api/core/firmware/status` (Version, ausstehende Updates), `/api/core/firmware/info` (Version auch dann, wenn nichts ansteht), `/api/diagnostics/interface/get_interface_statistics` (ältere Fassungen: `getInterfaceStatistics`), `/api/interfaces/overview/export` (Verbindungszustand, Beschreibung, MTU **und die Adressen samt IP-Aliasen** — fehlt auf älteren Fassungen), `/api/routes/gateway/status`, `/api/diagnostics/firewall/pf_statistics/state`, `/api/diagnostics/interface/get_vip_status` (CARP), `/api/wireguard/service/show`, HAProxy-Plugin für Backend-Zustände |
| Adressen nach außen | Aus `overview/export`: je Anschluss die eigene Adresse (v4 und v6), **alle IP-Aliase auf derselben Schnittstelle** und das Gateway. Als Anschluss nach außen gilt, **woran ein Gateway hängt** — nicht, was „WAN" heißt: ein zweiter Uplink trägt selten diesen Namen. Nur wenn keine Schnittstelle ein Gateway meldet, entscheidet die Kennung `wan`. Eine Adresse mit `vhid` ist eine CARP-Adresse und gehört dem Gerät nur, solange es MASTER ist. Eine private Adresse (RFC 1918, CGNAT, ULA) wird als solche gekennzeichnet: sie ist die Wahrheit über die Schnittstelle und **nicht** die Adresse, unter der der Standort im Internet zu finden ist
...
| | |
|---|---|
| Zugang | Benutzer und Passwort eines **lokalen** Kontos mit der Rolle *Viewer* — der Weg, der auf jeder Fassung funktioniert **oder** ein API-Schlüssel im Kopf `X-API-KEY`, sofern der Controller ihn anbietet (Network 9+, unter *Control Plane → Integrations*; auf selbst betriebenen Installationen längst nicht immer da). Der Schlüssel erbt die Rolle seines Kontos: er ist genau dann nur lesend, wenn das Konto es ist |
| Zwei Bauarten | UniFi OS (Dream Machine, Cloud Key Gen2, UNVR) hört auf **443**, hängt die Netzanwendung unter `/proxy/network` und meldet unter `/api/auth/login` an; eine selbst betriebene Network Application hört auf **8443**, kennt kein Präfix und meldet unter `/api/login` an. Nichts davon ist von außen zu sehen, also wird geprobt: fehlt in der Adresse der Port, gelten 443 und 8443, und auf jedem beide Anmeldepfade. Was getragen hat, merkt sich die Sitzung |
...
| Fallstrick | **Dieselbe 401, zwei Bedeutungen.** Die eigenständige Anwendung schützt alles unter `/api/` mit demselben Wachposten: einen Pfad, den sie nicht kennt — `/api/auth/login` von UniFi OS etwa — weist sie mit 401 und `api.err.LoginRequired` ab, also mit demselben Code wie ein falsches Passwort. Wer den Code für die Auskunft hält, versucht `/api/login` nie und meldet jemandem, sein Passwort sei falsch, während es stimmt. Unterschieden wird deshalb am **Rumpf**, nicht am Code |
...
| Fallstrick | **Der POST muss aussehen, als käme er von der eigenen Seite.** Neuere Fassungen der Network Application prüfen bei jedem POST `Referer`, `Origin` und das `csrf_token`, das die Anmeldeseite als Keks ausstellt. Ein Browser schickt das von selbst mit, ein Dienst muss es sagen. Fehlt es, kommt **dieselbe Absage wie bei einem falschen Passwort** — und dann steht im Leitstand „Zugangsdaten abgelehnt", während dieselben Daten in der Weboberfläche anstandslos hineinkommen. Der Sammler holt deshalb erst die Anmeldeseite und meldet sich dann mit Herkunft und Token an |
| Fallstrick | **Eine geglückte Anmeldung antwortet nicht immer mit 200.** Manche Fassungen leiten auf die Oberfläche um (302) und legen die Sitzung trotzdem bei. Was zählt, ist der ausgestellte Sitzungskeks (`TOKEN` oder `unifises`), nicht der Statuscode — ein `csrf_token` allein ist keine Sitzung |
| Wenn es trotzdem klemmt | `docker exec -it leitstand node /app/tools/unifi-probe.mjs --url https://… --user …` probiert jede Adresse, beide Anmeldepfade und drei Anfrageformen (nackt · mit Herkunft · mit Herkunft und CSRF-Token) und zeigt zu jedem Versuch Statuscode, ausgestellte Kekse und den Anfang der Antwort. Das beantwortet die Frage, die eine 401 offenlässt: meint der Controller das Konto oder die Form der Anfrage? |
